...
export const ADAPTERS = {
  ...
  codex: {
    ...
    install(packageRoot, command) {
      ...
      const pattern = /^\[mcp_servers\.eastmoney-quant\]\n(?:(?!\[)[^\n]*\n?)*/m
    ...
    uninstall(removeConfig = true) {
      const path = this.configPath()
      if (removeConfig && existsSync(path)) {
        const content = readFileSync(path, "utf8").replace(/^\[mcp_servers\.eastmoney-quant\]\n(?:(?!\[)[^\n]*\n?)*/m, "").trimEnd()
        writeFileSync(path, content ? `${content}\n` : "")
      }
      return { configPath: path, skills: removeSkills(packageRoot, this.skillRoot()) }
    },
  },
  cursor: jsonAdapter({
    id: "cursor",
