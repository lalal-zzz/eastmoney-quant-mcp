...
export async function install({ packageRoot, agents = "auto", dryRun = false, setupDataRoot }) {
  ...
  for (const adapter of selected) {
    ...
    state.agents[adapter.id] = { ...installed, backup: saved, installed_at: new Date().toISOString() }
...
export function uninstall(agent) {
  ...
  for (const id of ids) {
    ...
    // Restore only the backup made immediately before this package changed the config.
    const restored = Boolean(record.backup && existsSync(record.backup))
    if (restored) writeFileSync(record.configPath, readFileSync(record.backup))
    const adapter = ADAPTERS[id]
    if (adapter) adapter.uninstall(!restored)
  ...
  writeState(state)
  return { removed, dataPreserved: true }
}
